...
**배포 검증 — 커밋 `bce2405`·`92848a4` → origin/main → Vercel 자동 배포. 판정 PASS.**

- CI run 33046544532 (SHA 92848a4) → **success.** 로그에 `> node --test 'src/**/*.test.ts'`와 케이스 통과가 찍혀 Node 24 승격이 실제로 돌았음을 확인(스텝 스킵 아님).
- 운영 번들 `index-KeLZsuNC.js` → **`index-ByAVQkBa.js`**. 구 번들은 404, 신 번들은 200 — CDN 캐시와 무관한 증거.
- 운영에서 D1 링크 직접 열어 3연동 문·벽·소품 정상 렌더, 제품은 기본값 폴백, `치수 — 1250mm · A구간`, **콘솔 예외 0**.
- D5는 별도 운영 확인을 하지 않았다. 로컬 검증이 돌아간 번들 해시(`index-ByAVQkBa.js`)가 운영이 지금 서빙하는 것과 같아 산출물이 동일하다.
