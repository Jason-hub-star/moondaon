...
/** dev 전용 씬 저장 — ?edit=1 편집기가 POST한 코드 블록으로 props.data.ts(배치 SSOT)의 마커 구간을 재작성한다. */
function sceneSave(): Plugin {
  ...
  return {
    ...
    handleHotUpdate(ctx) {
      if (ctx.file.endsWith('scene/props.data.ts') && Date.now() - justSaved < 2000) {
      ...
      server.middlewares.use('/__scene-save', (req, res) => {
        ...
        req.on('end', () => {
          try {
            ...
            const file = path.resolve(ROOT, 'src/scene/props.data.ts')
